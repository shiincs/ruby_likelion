class ChatRoomsController < ApplicationController
  ...
  def create
    ...
    @chat_room.master_id = current_user.email # 방을 만든 사람이 방장
  ...
  def user_admit_room
   ...
   if current_user.joined_room?(@chat_room)
     # 이미 조인되어있는 유저?
     # 유저가 참가하고 있는 방의 목록중에 이 방이 포함?
     # current_user.chat_rooms.where(id: params[:id])[0].nil? # where는 배열에서 값을 찾는 
      render js: "alert('이미 참여한 방입니다.')"
     # 방에 참가하고 있는 유저들 중에 이 유저가 포함?
   else
    @chat_room.user_admit_room(current_user)
   end  
  end
